...
function applyFilters(){
  ...
  if(companiesSelected.length >= 2){
    // comparativo por empresa (gráfico 1)
    const comp = aggregateMonthlyByCompany(filtered);
    drawCharts(comp, true);
  } else {
    // modo normal (agregado total)
...
function drawCharts(data, comparative){
  ...
  // ---------------------------
  if(!comparative){
    // data é o array do aggregateMonthly()
  ...
  // data é o objeto do aggregateMonthlyByCompany()
  // ---------------------------
  const x = data.months; // labels YYYY-MM-DD

  const traces = [];

  // barras por empresa (agrupadas por mês)
  data.companies.forEach((emp, idx) => {
  ...
  // linha % total combinado (para não poluir com várias linhas)
  traces.push({
    x,
    y: data.totalByMonth.map(d => d.pct * 100),
    yaxis: "y2",
    type: "scatter",
    mode: "lines+markers",
    name: "% corte (total)",
  ...
  // Gráfico 2: breakdown continua sendo do total filtrado (mesmo com empresas selecionadas)
  // Então usamos totalByMonth como base do stack por modalidade:
  // (para isso, vamos montar um "fake" data com ENE/CNF/REL/SEM)
  // OBS: como o aggregateMonthlyByCompany não carrega ENE/CNF/REL/SEM, a quebra por modalidade
  // continua correta porque o filtro já selecionou empresas/usinas, mas precisa usar aggregateMonthly().
  // Para manter simples: só refazemos o breakdown a partir do RAW filtrado no applyFilters na próxima iteração.
  // Por ora, deixamos o breakdown igual ao último válido (ou você me pede e eu ajusto pra ficar perfeito).
}
